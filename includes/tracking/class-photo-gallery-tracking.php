...
class Hugeit_Photo_Gallery_Tracking
{
    ...
    {
        if (isset($_GET['hugeit_photo_gallery_tracking_opt_in'])) {
            add_action('admin_init', array($this, 'maybe_opt_in'));
        }

        add_action('admin_notices', array($this, 'admin_notice'));
        add_action('hugeit_photo_gallery_opt_in_cron', array($this, 'track_data'));
    }
    ...
    public function maybe_opt_in()
    {
        ...
        $opt_in_action = htmlspecialchars($_GET['hugeit_photo_gallery_tracking_opt_in']);
        ...
        header('Location: ' . admin_url('admin.php?page=photo_gallery_wp_gallery'));
    ...
    public function admin_notice()
    {
        ...
        if (!in_array($screen_id, Photo_Gallery_WP()->admin->get_pages())) return;
        ...
        echo Photo_Gallery_WP_Template_Loader::render(PHOTO_GALLERY_WP_TEMPLATES_PATH . '/admin/tracking/banner/show.php', compact('optin_url', 'optout_url'));
    }
    ...
    {
        return add_query_arg('hugeit_photo_gallery_tracking_opt_in', 1, admin_url('admin.php?page=photo_gallery_wp_gallery'));
    }
    ...
    {
        return add_query_arg('hugeit_photo_gallery_tracking_opt_in', 0, admin_url('admin.php?page=photo_gallery_wp_gallery'));
    }
    ...
    {
        return ('opted_in' === get_option('hugeit_photo_gallery_allow_tracking', false));
    }
    ...
    {
        return ('opted_out' === get_option('hugeit_photo_gallery_allow_tracking', false));
    }
    ...
    {
        update_option('hugeit_photo_gallery_allow_tracking', 'opted_in');
    }
    ...
    {
        update_option('hugeit_photo_gallery_allow_tracking', 'opted_out');
    }
    ...
    public function track_data()
    {
        ...
        $data["project_id"] = Photo_Gallery_WP()->get_project_id();
        $data["project_plan"] = Photo_Gallery_WP()->get_project_plan();
        $data["project_version"] = Photo_Gallery_WP()->get_version();
